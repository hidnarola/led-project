...
export class DeleteScheduleComponent implements OnInit {
  ...
  filePropertiesList = [];
  activeIndex = 0;
  constructor(private spinner: NgxSpinnerService, private notifier: NotifierService, private router: Router,
    private usservice: UserSignService, private service: SchedulesService) {
  }
  ngOnInit() {
    this.items = [
      { label: 'Select Sign' },
